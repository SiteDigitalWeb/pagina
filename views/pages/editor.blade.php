...
(function(editor) {
  ...
  editor.addComponents(`
    <style>
      ...
      .gjs-comp-selected .carousel-inner,
      .gjs-comp-hover .carousel-inner {
        display: block !important;
      }
      ...
      /* Indicadores y controles ocultos en vista previa */
      .carousel-indicators,
      .carousel-control {
        display: none;
      }
      
      .gjs-comp-selected .carousel-indicators,
      .gjs-comp-selected .carousel-control,
      .gjs-comp-hover .carousel-indicators,
      .gjs-comp-hover .carousel-control {
        display: block;
      }
      
      /* Asegurar que las imágenes sean visibles en el editor */
      .gjs-selected .carousel-inner img,
      .gjs-hovered .carousel-inner img {
        opacity: 1 !important;
  ...
  domc.addType('bootstrap-carousel', {
    ...
    model: {
      defaults: {
        ...
        script: function() {
          // Inicializar el carrusel cuando se añade al canvas
          if (typeof $ === 'function') {
        ...
        traits: [
          ...
          {
            type: 'select',
            label: 'Slide a editar',
            name: 'currentSlide',
            options: [{ value: 1, name: 'Slide 1' }],
            changeProp: 1,
          },
          { type: 'text', name: 'image', label: 'Imagen', changeProp: 1 },
          { type: 'text', name: 'title', label: 'Título', changeProp: 1 },
          { type: 'text', name: 'desc', label: 'Descripción', changeProp: 1 },
          { type: 'text', name: 'btnText', label: 'Texto Botón', changeProp: 1 },
          { type: 'text', name: 'btnUrl', label: 'URL Botón', changeProp: 1 },
          {
            type: 'checkbox',
            label: 'Autoplay',
            name: 'autoplay',
      ...
      init() {
        this.listenTo(this, 'change:slideCount', this.handleSlideCountChange);
        this.listenTo(this, 'change:currentSlide', this.loadCurrentSlideData);
        this.listenTo(this, 'change:autoplay change:interval change:animation', this.updateCarouselSettings);
        this.on('change:image change:title change:desc change:btnText change:btnUrl', this.updateCurrentSlideData);
        
        // Inicializar con los datos existentes si hay más de un slide
        const slidesData = this.get('slidesData');
      ...
      initializeCarousel() {
        const carouselId = this.get('attributes').id;
        
        this.components().reset([
          this.createIndicators(carouselId),
          this.createCarouselInner(),
          this.createControl('left', 'prev', carouselId),
          this.createControl('right', 'next', carouselId)
        ]);
      ...
      createControl(direction, slideType, carouselId) {
        return {
          tagName: 'a',
          attributes: {
            ...
            href: `#${carouselId}`,
            role: 'button',
            'data-slide': slideType,
          },
          components: [
            { 
              tagName: 'span', 
              attributes: { 
                class: `glyphicon glyphicon-chevron-${direction === 'left' ? 'left' : 'right'}`, 
                'aria-hidden': 'true' 
              } 
            },
            { 
              ...
              content: direction === 'left' ? 'Anterior' : 'Siguiente' 
            },
          ],
        };
      },
      ...
      updateSlideOptions() {
        const count = this.get('slideCount');
        const options = [];
        
        for (let i = 1; i <= count; i++) {
          options.push({ value: i, name: `Slide ${i}` });
        }
        
        this.getTrait('currentSlide').set('options', options);
      },

      handleSlideCountChange() {
        const newCount = this.get('slideCount');
        const currentCount = Object.keys(this.get('slidesData')).length;
        const slidesData = {...this.get('slidesData')};
        
        if (newCount > currentCount) {
          for (let i = currentCount + 1; i <= newCount; i++) {
            const slideKey = `slide${i}`;
            if (!slidesData[slideKey]) {
              slidesData[slideKey] = {
                image: `https://placehold.co/800x400?text=Slide+${i}`,
                title: `Título ${i}`,
                desc: `Descripción ${i}`,
                btnText: 'Ver más',
                btnUrl: '#'
          ...
          for (let i = newCount + 1; i <= currentCount; i++) {
            const slideKey = `slide${i}`;
            delete slidesData[slideKey];
          }
        }
        
        this.set('slidesData', slidesData);
        this.updateSlideOptions();
        
        if (this.get('currentSlide') > newCount) {
          this.set('currentSlide', newCount);
        }
      ...
      updateSlides() {
        const slidesData = this.get('slidesData');
        const carouselInner = this.find('.carousel-inner')[0];
        const indicators = this.find('.carousel-indicators')[0];
        const carouselId = this.get('attributes').id;
        
        if (!carouselInner || !indicators) return;
        
        carouselInner.components().reset();
        indicators.components().reset();
        
        Object.keys(slidesData).forEach((slideKey, index) => {
          const slideNum = index + 1;
          const slideData = slidesData[slideKey];
          const isActive = slideNum === 1;
          
          carouselInner.components().add({
            tagName: 'div',
            attributes: { 
            ...
            components: [
              ...
              },
              {
                ...
                attributes: { 
                  ...
                  style: 'position: absolute; bottom: 20px; left: 50%; transform: translateX(-50%); width: 80%; text-align: center;' 
                },
                components: [
                  { 
                    tagName: 'h3', 
                  ...
                  { 
                    tagName: 'p', 
                    content: slideData.desc,
                    style: 'margin: 10px 0; color: white;' 
                  },
                  {
                    tagName: 'a',
                    attributes: { 
                      href: slideData.btnUrl, 
                      class: 'btn btn-primary',
                      style: 'color: white; background-color: #007bff; border: none; padding: 5px 15px; text-decoration: none; display: inline-block;' 
                    },
                    content: slideData.btnText,
                  },
                ],
              },
            ],
          ...
          indicators.components().add({
            ...
            attributes: {
              ...
              style: 'width: 12px; height: 12px; border-radius: 50%; border: 1px solid #007bff;'
            },
          });
        });
      ...
      loadCurrentSlideData() {
        const currentSlide = this.get('currentSlide');
        const slidesData = this.get('slidesData');
        const slideKey = `slide${currentSlide}`;
        const slideData = slidesData[slideKey] || {};
        
        this.set('image', slideData.image || '');
        this.set('title', slideData.title || '');
        this.set('desc', slideData.desc || '');
        this.set('btnText', slideData.btnText || '');
        this.set('btnUrl', slideData.btnUrl || '');
      },
      ...
      updateCurrentSlideData() {
        ...
        slidesData[slideKey] = {
          ...
          desc: this.get('desc') || '',
          btnText: this.get('btnText') || '',
          btnUrl: this.get('btnUrl') || '',
        };
        ...
        const slide = carouselInner.find(`[data-slide-num="${currentSlide}"]`)[0];
        if (!slide) return;
        ...
        const caption = slide.find('.carousel-caption')[0];
        if (caption) {
          const h3 = caption.find('h3')[0];
          const p = caption.find('p')[0];
          const a = caption.find('a')[0];
          
          if (h3) h3.set('content', slideData.title);
          if (p) p.set('content', slideData.desc);
          if (a) {
            a.set('content', slideData.btnText);
            a.addAttributes({ 
              href: slideData.btnUrl,
              style: 'color: white; background-color: #007bff; border: none; padding: 5px 15px; text-decoration: none; display: inline-block;' 
            });
      ...
      updateCarouselSettings() {
        ...
        const attrs = {
          id: carouselId,
          'data-ride': autoplay ? 'carousel' : null,
          'data-interval': autoplay ? interval : null
        };
        
        let classes = 'carousel slide';
        if (animation === 'fade') classes += ' fade';
    ...
    view: {
      init() {
        this.listenTo(this.model, 'change:attributes change:slidesData', this.render);
      },
      
      onRender() {
        const inner = this.el.querySelector('.carousel-inner');
        if (inner && !inner.querySelector('.item')) {
          const slidesData = this.model.get('slidesData');
          const hasSlides = Object.keys(slidesData).length > 0;
          
          if (!hasSlides) {
            inner.innerHTML = `
              <div class="item active" style="position: relative; min-height: 150px; display: flex; justify-content: center; align-items: center;">
                <img src="https://placehold.co/800x400?text=Carrusel+Preview" alt="Preview" style="width: 100%; height: auto; display: block;">
                <div class="carousel-caption" style="position: absolute; bottom: 20px; left: 50%; transform: translateX(-50%); width: 80%; background: rgba(0,0,0,0.5); padding: 10px; border-radius: 5px; text-align: center;">
                  <h3 style="margin: 0; color: white;">Título de ejemplo</h3>
                  <p style="margin: 10px 0; color: white;">Descripción de ejemplo</p>
                  <a href="#" class="btn btn-primary" style="color: white; background-color: #007bff; border: none; padding: 5px 15px; text-decoration: none; display: inline-block;">Botón</a>
                </div>
              </div>
            `;
          }
        ...
        const images = this.el.querySelectorAll('.carousel-inner img');
        images.forEach(img => {
          img.style.opacity = '1';
  ...
  // Configuración del bloque para mejor arrastre
  editor.BlockManager.add('bootstrap-carousel', {
    label: 'Carrusel Bootstrap',
    category: 'Bootstrap',
    content: {
